...
# ── Disease classes ─────────────────────────────────────────────────────────────
# Classes are read directly from the loaded model (model.names) so this works
# for any trained checkpoint regardless of how many classes it has (e.g. the
# 14-class Roboflow Eye-Disease dataset used in this notebook).
_EMOJI_PALETTE = ["🔵", "🟡", "🟢", "🔴", "🟣", "🟠", "⚪", "🟤", "🔷", "🔶", "🟦", "🟧", "🟩", "🟥"]
_COLOR_PALETTE = [
    "#4B9FFF", "#FFD54B", "#4BFF91", "#FF4B4B", "#B14BFF", "#FF914B",
    "#E0E0E0", "#A1684B", "#4BD2FF", "#FFB14B", "#4B6FFF", "#FF7A4B",
    "#7AFF4B", "#FF4BA1",
]
NORMAL_KEYWORDS = ("normal", "healthy", "no_disease", "none")

def _is_normal_class(label: str) -> bool:
    return any(k in label.lower() for k in NORMAL_KEYWORDS)

def build_class_info(names_dict):
    info = {}
    for idx, label in names_dict.items():
        if _is_normal_class(label):
            info[label] = {"emoji": "🟢", "color": "#4BFF91", "desc": "No condition detected"}
        else:
            info[label] = {
                "emoji": _EMOJI_PALETTE[idx % len(_EMOJI_PALETTE)],
                "color": _COLOR_PALETTE[idx % len(_COLOR_PALETTE)],
                "desc": "",
            }
    return info

CLASS_INFO = build_class_info(model.names)
...
with gr.Blocks(theme=theme, css=css, title="EyeCare AI") as demo:
    ...
    with gr.Column(elem_id="app-wrapper"):
            ...
            <div class="header-left">
                ...
                <div>
                    ...
                    <p class="subtitle">
                        ...
                        Smart Eye Disease Detection &nbsp;·&nbsp; YOLOv8 Deep Learning
                    </p>
                </div>
            </div>
        ...
        <div id="stats-strip">
            ...
            <div class="stat-chip">
                <div class="stat-num">Roboflow</div>
                <div class="stat-lbl">Dataset Source</div>
            </div>
            <div class="stat-chip">
                <div class="stat-num">640px</div>
        ...
        with gr.Row(elem_id="main-grid", equal_height=True):
            ...
            with gr.Column(elem_id="left-panel", elem_classes="panel"):
                ...
                # Class legend (built dynamically from the model's actual classes)
                _legend_chips = "".join(
                    f'<span class="legend-chip" style="background:{info["color"]};">{info["emoji"]} {label}</span>'
        ...
        <div id="bottom-bar">
            <span class="disclaimer-text">
                ...
                EyeCare AI is for <strong>research &amp; screening purposes only</strong>.
                Not a substitute for professional ophthalmological diagnosis.
                Always consult a licensed eye-care specialist.
            </span>
